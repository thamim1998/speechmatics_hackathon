                            ...
                            if event_type == "content_streaming":
                                ...
                                try:
                                    usage = CompletionUsage(
                                        completion_tokens=parsed.get("output_tokens") or total_tokens or 0,
                                        prompt_tokens=parsed.get("input_tokens") or 0,
                                        total_tokens=parsed.get("total_tokens") or total_tokens or 0,
                                    )
                                    self._event_ch.send_nowait(ChatChunk(id=request_id, usage=usage))
                                except Exception as e:
                                    logger.warning(f"Failed to parse usage stats: {e}")
                                return
